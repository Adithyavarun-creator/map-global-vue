...
am5.ready(function () {
  ...
  // const url = "/api/Country/getImsGeoCoordinates";
  ...
  onMounted(() => {
    const getCities = async (url) => {
      ...
      for (var i = 0; i < cities.length; i++) {
        var city = cities[i];
        addCity(city.longitude, city.latitude, city.name);
        //change city.name to city.title in asp dot net
      }
